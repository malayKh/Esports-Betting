...
contract TournamentSubContract is AccessControl {
    ...
    function registerTeam(address _manager, address[] memory _players, uint256 entryFeeAfterTax) external onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    function distributePrizes(address[] memory _winningManagers, uint256[] memory _prizeAmounts) external onlyRole(DEFAULT_ADMIN_ROLE) {
        ...
        for (uint256 i = 0; i < _winningManagers.length; i++) {
            ...
            require(paymentToken.transfer(_winningManagers[i], captainShare), "Captain prize transfer failed");
            
            // Distribute to other players
            for (uint256 j = 0; j < TEAM_SIZE; j++) {
